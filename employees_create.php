<!-- Página de criação de funcionário -->
<?php
include('protect.php');
?>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    do {
        ...
        if (empty($name) || empty($email) || empty($phone) || empty($address) || empty($client_id)) {
            $erro = "Todos os campos são obrigatórios.";
            break;
        }

        //verificando se o cliente com o ID fornecido existe no banco de dados
        $check_client_sql = "SELECT id FROM clients WHERE id = $client_id";
        $check_client_result = $connection->query($check_client_sql);

        if (!$check_client_result || $check_client_result->num_rows == 0) {
            $erro = "O cliente com o ID $client_id não existe. Insira um ID de cliente válido.";
            break;
        }

        try {
            //inserindo os dados na tabela clients
            $sql = "INSERT INTO employees (name, email, phone, address, client_id) " .
                "VALUES ('$name', '$email', '$phone', '$address', '$client_id')";
            ...
            //redirecionando o usuário novamente ao painel de funcionários
...
<head>
    ...
    <script>
        function cancelar() {
            window.location.href = "/somativa2/employees.php";
        }
    </script>    
</head>
...
<body>
    <div class="container my-5">
        ...
        <form method="post">
            ...
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label">ID do Cliente</label>
                <div class="col-sm-6">
                    <input type="number" class="form-control" name="client_id" value="<?php echo $client_id; ?>" min="1">
                </div>
            </div>
